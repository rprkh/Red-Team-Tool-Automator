...
def main():
    ...
    http_opt_method = get_user_input("Enter the URL")
    if http_opt_method:
        http_opt_method = get_flag_value("Enter the URL")
    ...
    try:
        subprocess.run(command + " -c -w wfuzz/wordlist/general/common.txt " + str(http_opt_method), shell=True, check=True)
        subprocess.run(command + " -w wfuzz/wordlist/general/common.txt --hc 404 " + str(http_opt_method), shell=True, check=True)
        subprocess.run(command + " -w wfuzz/wordlist/general/common.txt --sc 404 " + str(http_opt_method), shell=True, check=True)
        subprocess.run(command + " -z file,wfuzz/wordlist/general/common.txt " + str(http_opt_method), shell=True, check=True)
