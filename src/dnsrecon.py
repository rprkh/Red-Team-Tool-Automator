...
def main():
    ...
    try:
        subprocess.run(command + " -t std", shell=True, check=True)
        subprocess.run(command + " --threads 2 --lifetime 5 -t std", shell=True, check=True)
        subprocess.run(command + " -x dnsrecon.xml -v -t zonewalk", shell=True, check=True)
        subprocess.run(command + " -t std", shell=True, check=True)
        subprocess.run(command + " -c dnsrecon.csv -v -t zonewalk", shell=True, check=True)
