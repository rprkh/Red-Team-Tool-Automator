...
# Check if Photon is already installed
if [ -d "Photon" ]; then
    echo "Photon has already been successfully installed on your system."
else
    ...
    if [ $? -ne 0 ]; then
        echo "Error: Failed to install the dependencies for Photon."
        exit 1
    fi

    python3 photon.py -h

    echo "Photon installation and setup successfully completed"
